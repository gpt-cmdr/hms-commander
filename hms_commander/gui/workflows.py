"""Deterministic HMS GUI workflow helpers."""
...
def project_state_file(version: str) -> Path:
    """Return the HMS project-state file for a version such as ``4.12``."""
    digits = "".join(part for part in version if part.isdigit())
    if not digits:
...
def seed_startup_project(
    ...
    *,
    ...
    content = state.read_text(encoding="utf-8") if original_state_existed else ""
    ...
    content = _replace_or_insert_block(content, "Project", block_body)
    content = _replace_or_insert_block(content, "Recent Projects", recent_body)
...
def startup_project_seed(
    ...
    *,
    ...
    """Temporarily seed HMS startup state and restore it on exit."""
...
def launch_hms(
    *,
    ...
    """Launch HMS, optionally seeding the startup project first."""
    ...
    jre_bin = resolve_jre_bin(exe.parent, version=version)
    if jre_bin is None:
        raise HmsGuiUnavailableError(f"Could not resolve HMS JRE bin for {exe.parent}")
    jabswitch = jre_bin / "jabswitch.exe"
    if jabswitch.exists():
        subprocess.run([str(jabswitch), "/enable"], check=False, capture_output=True)
